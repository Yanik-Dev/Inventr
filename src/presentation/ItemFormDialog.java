...
public class ItemFormDialog extends javax.swing.JDialog {
     ...
     * Creates new form ItemDialog
     */
    public ItemFormDialog(java.awt.Frame parent, boolean modal, Item item) {
        super(parent, modal);
        initComponents();
        this.getSuppliers();
        this.getCategories();
    ...
    public void getCategories(){
        ...
        for(Category category : this.categories){
            this.comboBoxCategory.addItem(category.getName());
        }
    }
    ...
    public boolean validateForm(){
        ...
        try{
           Double d = Double.parseDouble(this.textBoxUnitCost.getText().trim());
           if(d < 1){
            counter++;
            errorMessage += counter+". Unit cost must be over 0\n";
           }
        }
        catch(NumberFormatException ex){
           counter++;
           errorMessage += counter+". Unit cost must be a number\n";
           this.textBoxUnitCost.setText("0.00");
        }
        
        try{
           Double d = Double.parseDouble(this.textBoxReorderLevel.getText().trim());
           if(d < 1){
            counter++;
            errorMessage += counter+". Reorder Level must be over 0\n";
           }
        }
        catch(NumberFormatException ex){
           counter++;
           errorMessage += counter+". Reorder Level must be a number\n";
    ...
    private void buttonActionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonActionActionPerformed
        ...
        if(this.isUpdate){
            ...
            result = this._dbService.exist(this.item);
            if(result){
                MessageBox.infoBox(this, "- Result", "Item "+this.item.getItemName()+" already exist.");
                return;
            }
